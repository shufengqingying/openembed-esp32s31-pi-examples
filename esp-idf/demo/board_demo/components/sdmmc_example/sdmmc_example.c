 ...
 * @brief SDMMC card example with FAT filesystem support
 *
 * This module demonstrates SD/MMC card initialization and basic file operations
 * using the SDMMC peripheral in 1-bit or 4-bit mode.
 *
 * Features:
 * - SD/MMC card detection and initialization via SDMMC host
 * - FAT filesystem mounting with optional auto-format on failure
 * - Card information printing (name, type, speed, capacity)
 * - Basic file operations: create, write, read, rename
 * - Configurable bus width (1-line or 4-line) and speed mode
 * - Support for UHS-I cards (SDR50, DDR50, SDR104) on supported SoCs
 * - On-chip LDO power control for SD card power supply (when available)
 * - Optional GPIO debugging with ADC pin monitoring
 *
 * Configuration options (menuconfig):
 * - CONFIG_EXAMPLE_FORMAT_IF_MOUNT_FAILED: Auto-format card on mount failure
 * - CONFIG_EXAMPLE_FORMAT_SD_CARD: Format card as part of the example
 * - CONFIG_EXAMPLE_SDMMC_BUS_WIDTH: 1-line or 4-line mode
 * - CONFIG_EXAMPLE_SDMMC_SPEED_MODE: Default Speed, High Speed, or UHS-I modes
 * - CONFIG_EXAMPLE_PIN_*: GPIO pin assignments for SDMMC signals
 * - CONFIG_EXAMPLE_SD_PWR_CTRL_LDO_INTERNAL_IO: Internal LDO power control
 *
 * Note: External 10k pull-up resistors are required on all data lines for
 * reliable operation, especially in 4-bit mode.
 */
...
/**
 * @brief Initialize the SDMMC example
 */
void sdmmc_example_init(void)
{
    ESP_LOGI(TAG, "Initializing SDMMC example...");
    ...
    ESP_LOGI(TAG, "SDMMC example initialized successfully");
}
