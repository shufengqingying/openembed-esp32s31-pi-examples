 ...
 * @brief USB MSC (Mass Storage Class) example with FAT filesystem support
 *
 * This module demonstrates USB Host mode with Mass Storage Class (MSC) devices,
 * commonly known as USB flash drives or external storage devices.
 *
 * Features:
 * - USB Host stack initialization and configuration
 * - MSC device detection with hot-plug support (connect/disconnect)
 * - Virtual File System (VFS) mount for FAT formatted USB drives
 * - Multiple MSC device support (up to CONFIG_FATFS_VOLUME_COUNT)
 * - Automatic device information printing (capacity, sector size, VID/PID)
 * - Basic file operations: directory creation, file write/read
 * - Sequential read/write speed test (1MB transfer)
 * - BOOT button (GPIO 0) press to exit the example cleanly
 *
 * Configuration options (menuconfig):
 * - CONFIG_USB_MSC_EXAMPLE_ENABLE: Enable/disable this component
 *
 * USB MSC events are processed in the main loop via usb_msc_example_run().
 * The USB background task handles Host Library events and MSC driver callbacks.
 *
 * Note: This component relies on GPIO ISR service being installed by the
 * button_input_example component. Do not call gpio_install_isr_service()
 * from this component to avoid ESP_ERR_INVALID_STATE.
 *
 * Note: File operations and speed tests may take several seconds to complete.
 * The main loop should call vTaskDelay() periodically to feed the task watchdog.
 *
 * Note: Wide character string descriptors (iProduct, iManufacturer, iSerialNumber)
 * may not display correctly when CONFIG_LIBC_NEWLIB_NANO_FORMAT is enabled.
 */
...
static const char *TAG = "usb_msc_example";
 ...
 *
 * Processes pending USB events (connect/disconnect). This function
 * should be called periodically from the main loop.
 */
void usb_msc_example_run(void)
{
    // Only process events if the queue exists
    if (app_queue == NULL)
    {
        return;
    }
    ...
    if (msg.id == APP_DEVICE_CONNECTED)
    {
        ...
        ESP_LOGI(TAG, "========================================");
        ESP_LOGI(TAG, "USB flash drive connected and mounted successfully!");
        ESP_LOGI(TAG, "Mount point: " MNT_PATH "%d", slot);
        ESP_LOGI(TAG, "You can now access the drive via VFS");
        ESP_LOGI(TAG, "========================================");
    ...
    else if (msg.id == APP_DEVICE_DISCONNECTED)
    {
        ...
        if (slot >= 0)
        {
            ...
            ESP_LOGI(TAG, "USB flash drive disconnected, you can reconnect it");
        }
    }
    else if (msg.id == APP_QUIT)
    {
        ...
        msc_host_uninstall();
        ESP_LOGI(TAG, "USB MSC example exiting...");
...
/**
 * @brief Initialize the USB MSC example
 */
void usb_msc_example_init(void)
{
    ESP_LOGI(TAG, "Initializing USB MSC example...");
    ESP_LOGI(TAG, "========================================");
    ESP_LOGI(TAG, "USB MSC Example");
    ESP_LOGI(TAG, "Please insert a USB flash drive into the USB port");
    ESP_LOGI(TAG, "Press BOOT button to exit the example");
    ESP_LOGI(TAG, "========================================");
    ...
    // Note: GPIO ISR service is already installed by button_input_example component.
    // Do not call gpio_install_isr_service() here to avoid ESP_ERR_INVALID_STATE.
    ...
    ESP_LOGI(TAG, "USB MSC example initialized successfully");
}
